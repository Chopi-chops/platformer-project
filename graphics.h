...
void derive_graphics_metrics_from_loaded_level() {
    ...
    cell_size = screen_size.y / static_cast<float>(LevelController::get_instance().get_levels()[level_index].get_rows());
...
void draw_parallax_background() {
    ...
    float initial_offset      = -(Player::get_instance().get_player_posX() * PARALLAX_PLAYER_SCROLLING_SPEED + game_frame * PARALLAX_IDLE_SCROLLING_SPEED);
    ...
    for (size_t row = 0; row < LevelController::get_instance().get_current_level().get_rows(); ++row) {
        for (size_t column = 0; column < LevelController::get_instance().get_current_level().get_columns(); ++column) {
            Vector2 pos = {
                ...
                (static_cast<float>(column) - Player::get_instance().get_player_posX()) * cell_size + horizontal_shift,
...
void draw_player() {
    ...
    Vector2 pos = {
        horizontal_shift,
        Player::get_instance().get_player_posY() * cell_size
};
    ...
    if (game_state == GAME_STATE) {
        if (!(Player::get_instance().is_player_on_ground())) {
            ...
            Player::get_instance().set_is_moving(false);
        }
        else {
...
void draw_enemies() {
    ...
    for (auto &enemy : EnemiesController::get_instance().get_enemies()) {
        ...
        Vector2 pos = {
                (enemy.get_pos().x - Player::get_instance().get_player_posX()) * cell_size + horizontal_shift,
